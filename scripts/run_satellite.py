...
def train_scene(gpu, scene, factor):
    cmd = f"OMP_NUM_THREADS=4 CUDA_VISIBLE_DEVICES={gpu} python train.py -s {dataset_dir}/{scene}/outputs_skew -m {output_dir}/{scene}_{output_suffix} --eval --white_background --port {6209+int(gpu)} --kernel_size 0.1 --resolution 1 --sh_degree 1 --opacity_reset_interval 60000"
    print(cmd)
    if not dry_run:
        os.system(cmd)
